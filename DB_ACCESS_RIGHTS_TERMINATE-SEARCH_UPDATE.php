...
if(isset($_REQUEST))
{
    ...
    else if($_REQUEST['option']=='FETCH')
    {
        ...
        $select_recver="SELECT UA_REC_VER FROM USER_ACCESS WHERE ULD_ID =(SELECT ULD_ID FROM USER_LOGIN_DETAILS WHERE ULD_LOGINID='$loginid_result') AND UA_TERMINATE='X'";
        ...
        if(count($recver_array)==1){
            $query= "SELECT UA_REASON,UA_END_DATE ,UA_REC_VER FROM USER_ACCESS where ULD_ID =(select ULD_ID FROM USER_LOGIN_DETAILS WHERE ULD_LOGINID ='".$loginid_result."')";
        ...
        $query= "SELECT UA_REASON,UA_END_DATE  FROM USER_ACCESS where ULD_ID =(select ULD_ID FROM USER_LOGIN_DETAILS WHERE ULD_LOGINID ='".$loginid."') and UA_REC_VER='$recver'";
        $loginsearch_fetchingdata= mysqli_query($con, $query);
        $URT_SRC_values=array();
        if($row=mysqli_fetch_array($loginsearch_fetchingdata)){
            $URT_SRC_enddate=$row["UA_END_DATE"];
    ...
    else if($_REQUEST['option']=='UPDATE')
    {
        ...
        $sql="UPDATE USER_ACCESS SET UA_END_DATE='$enddate',UA_REASON='$reason_update',UA_USERSTAMP='$userstamp' where ULD_ID=(SELECT ULD_ID from USER_LOGIN_DETAILS where ULD_LOGINID='".$loggin."')  AND UA_REC_VER='$recver'";
    ...
    else if($_REQUEST['option']=='REJOIN')
    {
        ...
        if($flag==1){
            ...
            $return_array=URSRC_calendar_create($loggin,$fileId,$loginid_name,$URSC_uld_id,$joindate,$calenderid,'REJOIN DATE','REJOIN');
    ...
    else if($_REQUEST['option']=='TERMINATE')
    {
        ...
        if($flag==1){
            ...
            $return_array=URSRC_calendar_create($loggin,$fileId,$loginid_name,$URSC_uld_id,$enddate,$calenderid,'TERMINATE DATE','TERMINATE');
            $ss_flag=$return_array[0];
            $cal_flag=$return_array[1];
