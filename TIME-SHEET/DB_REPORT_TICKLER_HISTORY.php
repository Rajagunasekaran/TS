...
if($_REQUEST['option']=="search")
{
    ...
    //    echo ("CALL SP_TS_USER_ADMIN_REPORT_DETAILS_TICKLER_DATA('$userstamp',@TEMP_UARD_TICKLER_HISTORY)");
    $result = $con->query("CALL SP_TS_USER_ADMIN_REPORT_DETAILS_TICKLER_DATA('$USERSTAMP',@TEMP_UARD_TICKLER_HISTORY)");
    if(!$result) die("CALL failed: (" . $con->errno . ") " . $con->error);
    $select = $con->query('SELECT @TEMP_UARD_TICKLER_HISTORY');
    $result = $select->fetch_assoc();
    $temp_tickler_history= $result['@TEMP_UARD_TICKLER_HISTORY'];
    $TH_values=array();
    $sqlquery=mysqli_query($con,"SELECT EVENT_TYPE,TABLE_NAME,TH_OLD_VALUE,TH_NEW_VALUE,TH_USERSTAMP,DATE_FORMAT(TH_TIMESTAMP, '%d-%m-%Y %h:%m:%s') AS TH_TIMESTAMP FROM $temp_tickler_history WHERE ULD_ID='$TH_uld_id' and TABLE_NAME='USER_ADMIN_REPORT_DETAILS'");
