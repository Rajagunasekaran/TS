...
//FETCHING DATA TABLE FRM DB FOR PROJECT REVENUE
    if($_REQUEST['option']=="projectname")
    {
        ...
        $REV_startdate="null";
        $REV_enddate="null";
        $result = $con->query("CALL SP_PROJECT_REVENUE_BY_PROJECT_NAME('1','$projectname',$REV_startdate,$REV_enddate,'$USERSTAMP',@TEMP_PROJECT_REVENUE,@TOT_NO_OF_DAYS)");
        if(!$result) die("CALL failed: (" . $con->errno . ") " . $con->error);
        $select = $con->query('SELECT @TEMP_PROJECT_REVENUE');
        $result = $select->fetch_assoc();
        $temp_table_name= $result['@TEMP_PROJECT_REVENUE'];
        $totalnoof_days=$result['@TOT_NO_OF_DAYS'];
        $REV_values=array();
        $sqlquery=mysqli_query($con,"SELECT USERNAME,TOT_NO_OF_DAYS FROM $temp_table_name");
        $values_array=false;
        while($row=mysqli_fetch_array($sqlquery)){
            $username=$row['USERNAME'];
            $noofdays=$row['TOT_NO_OF_DAYS'];
            $REV_values=array('username'=>$username,'noofdays'=>$noofdays,'noofdays'=>$noofdays);
...
//FETCHING DATA TABLE FRM DB FOR PROJECT REVENUE BY ACTIVE NONACTIVE EMPLOYEE
    if($_REQUEST['option']=="nonactiveempdatatble")
    {
        ...
        $result = $con->query("CALL SP_PROJECT_REVENUE_BY_EMPLOYEE('$loginid','$USERSTAMP',@TEMP_PROJECT_EMPLOYEE_REVENUE,@NO_OF_PROJECT,@NO_OF_DAYS_WORKED)");
        if(!$result) die("CALL failed: (" . $con->errno . ") " . $con->error);
        $select = $con->query('SELECT @TEMP_PROJECT_EMPLOYEE_REVENUE,@NO_OF_PROJECT,@NO_OF_DAYS_WORKED');
        $result = $select->fetch_assoc();
        ...
        while($row=mysqli_fetch_array($select_data_rs)){
            ...
            $final_values=array('projectdate'=>$projectdate,'projectname'=>$projectname,'noof_prjct_day'=>$noofprjct_day,'working_day' => $working_day);
...
//FETCHING DATA TABLE FRM DB FOR PROJECT REVENUE BY ACTIVE NONACTIVE EMPLOYEE BY DATERANGE
    if($_REQUEST['option']=="non_activeemp_dterange")
    {
        ...
        while($row=mysqli_fetch_array($select_data_rs)){
            $projectdate=$row['PROJECT_DATE'];
            $projectname=$row['PROJECT_NAME'];
            $final_values=array('projectdate'=>$projectdate,'projectname'=>$projectname,'total_no_project'=>$total_no_project,'working_day' => $working_day);
            $values_array[]=$final_values;
        }
        $drop_query="DROP TABLE $temp_table_name ";
        mysqli_query($con,$drop_query);
