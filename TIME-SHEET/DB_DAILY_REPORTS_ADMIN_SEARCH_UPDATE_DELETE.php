...
where UARD_DATE BETWEEN '$startdate' AND '$enddate' and UARD.ULD_ID='$ure_uld_id' ORDER BY UARD.UARD_DATE ");
        while($row=mysqli_fetch_array($date)){
            ...
            $final_values=(object) ['id'=>$ure_id,'date' => $ure_date,'report' =>$ure_report,'userstamp'=> $ure_userstamp,'timestamp'=>$ure_timestamp,'reason'=>$ure_reason,'permission'=>$ure_permission,'attendance'=>$ure_attendance,'pdid'=>$ure_pdid,'morningsession'=>$ure_morningsession,'afternoonsession'=>$ure_afternoonsession,'bandwidth'=>$ure_bandwidth,'user_stamp'=>$userstamp,'flag'=>$ure_flag];
    ...
    if($_REQUEST['option']=='ALLDATE')
    {
        ...
        $sql=mysqli_query($con,"SELECT DISTINCT A.UARD_REPORT,A.UARD_REASON,B.ULD_LOGINID,C.ULD_LOGINID as USERSTAMP,DATE_FORMAT(CONVERT_TZ(A.UARD_TIMESTAMP,'+00:00','+05:30'), '%d-%m-%Y %T') as UARD_TIMESTAMP FROM USER_ADMIN_REPORT_DETAILS A
            INNER JOIN USER_LOGIN_DETAILS B on A.ULD_ID=B.ULD_ID INNER JOIN USER_LOGIN_DETAILS C on A.UARD_USERSTAMP_ID=C.ULD_ID
              INNER JOIN USER_ACCESS D where A.UARD_DATE='$empdate' and D.UA_TERMINATE IS null order by ULD_LOGINID");
        while($row=mysqli_fetch_array($sql)){
            ...
            $all_values=(object) ['admreason'=>$adm_reason,'admreport' =>$adm_report,'admuserstamp'=> $adm_userstamp,'admtimestamp'=>$adm_timestamp,'admlogin'=>$adm_loginid];
...
// for absent radio button
        ...
        $result = $con->query("CALL SP_TS_DAILY_REPORT_SEARCH_UPDATE($id,'$report','$reason','$finaldate',$ADM_urc_id,'$login_id','$perm_time','$ADM_attendance','$projectid','$uard_morning_session','$uard_afternoon_session',$bandwidth,'$USERSTAMP','$flag_abs',@success_flag)");
