...
if($Current_day!='Sunday'){
    if($check_ph==0 && $check_onduty==0){
        ...
        for($i=0;$i<$array_length;$i++){
            ...
            $message='<body>'.'<br>'.'<h> '.$sub.'</h>'.'<br>'.'<br>'.'<table border=1  width=200 ><thead  bgcolor=#6495ed style=color:white><tr  align=center  height=2px ><td >REPORT DATE</td></tr></thead>';
            ...
            if($last_date!=$currentdate){
                ...
                for($j=0;$j<count($date_array);$j++){
                    $message=$message. "<tr><td >".date('l jS  F Y ',strtotime($date_array[$j]))."</td></tr>";
                }
