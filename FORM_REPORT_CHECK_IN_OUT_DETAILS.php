...
<script>
    ...
    $(document).ready(function(){
        ...
        var option="common";
        xmlhttp.open("GET","DB_REPORT_CHECK_IN_OUT_DETAILS.do?option="+option);
        xmlhttp.send();
       // CHANGE EVENT FOR SEARCH BY LIST BX
        ...
        $(document).on('change','#REP_chk_rd_allactveemp',function(){
            ...
            xmlhttp.open("GET","DB_REPORT_CHECK_IN_OUT_DETAILS.do?REV_project_name="+REV_project_name+"&option="+choice,true);
        ...
        $(document).on('change','#REP_lb_loginid',function(){
            ...
            else
            {
                ...
                xmlhttp.open("GET","DB_REPORT_CHECK_IN_OUT_DETAILS.do?REP_chk_loginid="+REP_chk_loginid+"&option="+choice,true);
                xmlhttp.send(new FormData(formElement));
        ...
        $(document).on('click','#REP_btn_date',function(){
            ...
            xmlhttp.onreadystatechange=function() {
                if (xmlhttp.readyState==4 && xmlhttp.status==200) {
                    ...
                    if(allvalues_array.length!=0){
                        var REP_allactve_msg=REP_chk_errorAarray[5].toString().replace("[DATE]",date);
                        ...
                        $("#REP_btn_pdfs").show();
                            ...
                            for(var j=0;j<allvalues_array.length;j++){
                                ...
                                if((clockoutlocation=='null')||(clockinlocation==undefined))
                                {
                                    clockoutlocation='';
                                }
            ...
            xmlhttp.open("GET","DB_REPORT_CHECK_IN_OUT_DETAILS.do?option="+option+"&date="+date+"&loginid="+loginid);
            xmlhttp.send();
        });
        var allvalues_array;
        ...
        $(document).on('click','#REP_btn_searchdaterange',function(){
                ...
                if (xmlhttp.readyState==4 && xmlhttp.status==200) {
                    ...
                    if(allvalues_array.length!=0){
                        ...
                        $('#REP_lbl_daterange').text(REP_strtend_errmsgs).show();
                            var ADM_tableheader='<table id="REP_tble_absent_count" border="1"  cellspacing="0" class="srcresult" style="width:1000px" ><thead  bgcolor="#6495ed" style="color:white"><tr><th>DATE</th><th>CLOCK IN</th><th style="width:250px">CLOCK IN LOCATION</th><th>CLOCK OUT</th><th style="width:250px">CLOCK OUT LOCATION</th></tr></thead><tbody>'
                            for(var j=0;j<allvalues_array.length;j++){
            ...
            xmlhttp.open("GET","DB_REPORT_CHECK_IN_OUT_DETAILS.do?option="+option+"&startdate="+startdate+"&enddate="+enddate+"&loginid="+loginid);
            xmlhttp.send();
        });
        ...
        $(document).on('click','#REP_btn_pdf',function(){
            ...
            var url=document.location.href='COMMON_PDF.php?flag=24&inputValOne='+inputValOne+'&inputValTwo='+inputValTwo+'&inputValThree='+inputValThree+'&title='+REP_strtend_errmsgs;
        });
        //CLICK FUNCTION FOR PDF BUTTON
        $(document).on('click','#REP_btn_pdfs',function(){
            var inputValFour=$('#REP_tb_date').val();
            inputValFour = inputValFour.split("-").reverse().join("-");
            var url=document.location.href='COMMON_PDF.php?flag=23&inputValFour='+inputValFour+'&title='+REP_strtend_errmsgs;
        });
    });
    <!--SCRIPT TAG END-->
</script>
