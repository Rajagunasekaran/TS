...
{
 $select="SELECT A.ETD_ID,B.ET_ID,A.ETD_EMAIL_SUBJECT,A.ETD_EMAIL_BODY,B.ET_EMAIL_SCRIPT,C.ULD_LOGINID,A.ETD_TIMESTAMP FROM EMAIL_TEMPLATE_DETAILS A LEFT JOIN EMAIL_TEMPLATE B ON A.ET_ID = B.ET_ID LEFT JOIN USER_LOGIN_DETAILS C ON A.ULD_ID = C.ULD_ID";
 ...
 $y=$insert;
   ...
   while($insert=mysqli_fetch_array($record))
   {
      ...
      $ET_ENTRY_body=$insert["ETD_EMAIL_BODY"];
       ...
       $ET_ENTRY_timestamp=$insert["ETD_TIMESTAMP"];
       $ET_ENTRY_el_id=$insert['ETD_ID'];
       $ET_ENTRY_id=$insert['ET_ID'];
       $appendTable1.='<tr><td id=sname_'.$ET_ENTRY_id.' class="snameedit">'.$ET_ENTRY_scriptname.'</td><td id=emailsubject_'.$ET_ENTRY_el_id.' class="emailsubject">'.$ET_ENTRY_subject.'</td><td id=emailbody_'.$ET_ENTRY_el_id.' class="emailbody">'.$ET_ENTRY_body.'</td><td id=user_'.$ET_ENTRY_el_id.' class="user"> '.$ET_ENTRY_userstamp.'</td><td id=time_'.$ET_ENTRY_el_id.' class="time"> '.$ET_ENTRY_timestamp.'</td></tr>';
...
{
    $ET_ENTRY_id=$_REQUEST['ET_ID'];
    $ET_ENTRY_scriptname=$_REQUEST['scriptvalue'];
//        $Contact=$_REQUEST['contactvalue'];
//        $Address=$_REQUEST['addressvalue'];
//        $Date=$_REQUEST['datevalue'];


    $update="UPDATE EMAIL_TEMPLATE SET ET_EMAIL_SCRIPT='$ET_ENTRY_scriptname' WHERE ET_ID= $ET_ENTRY_id";
...
//        $Date=$_REQUEST['datevalue'];
    ...
    $update="UPDATE EMAIL_TEMPLATE_DETAILS SET ETD_EMAIL_SUBJECT= '$ET_ENTRY_subject' WHERE ETD_ID=  $ET_ENTRY_el_id";
//        $update="UPDATE emp SET Contact='$Contact' WHERE id=$uid";
//        $update="UPDATE emp SET Address='$Address' WHERE id=$uid";
//        $update="UPDATE emp SET Date='$Date' WHERE id=$uid";
    if ($con->query($update) === TRUE) {
...
//        $Date=$_REQUEST['datevalue'];
    ...
    $update="UPDATE EMAIL_TEMPLATE_DETAILS SET ETD_EMAIL_BODY='$ET_ENTRY_body' WHERE ETD_ID=  $ET_ENTRY_el_id";
//        $update="UPDATE emp SET Contact='$Contact' WHERE id=$uid";
//        $update="UPDATE emp SET Address='$Address' WHERE id=$uid";
//        $update="UPDATE emp SET Date='$Date' WHERE id=$uid";
    if ($con->query($update) === TRUE) {
