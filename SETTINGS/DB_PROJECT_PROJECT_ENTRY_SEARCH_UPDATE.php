...
{
    $select="SELECT A.PD_ID,B.PS_ID,C.PC_ID,D.ULD_ID, A.PD_PROJECT_NAME,B.PS_PROJECT_DESCRIPTION,B.PS_REC_VER,B.PS_START_DATE,B.PS_END_DATE,C.PC_DATA,D.ULD_LOGINID, A.PD_TIMESTAMP FROM PROJECT_DETAILS A LEFT JOIN PROJECT_STATUS B ON A.PD_ID = B.PD_ID LEFT JOIN PROJECT_CONFIGURATION C ON C.PC_ID=B.PC_ID LEFT JOIN USER_LOGIN_DETAILS D ON A.ULD_ID=D.ULD_ID ORDER BY PD_PROJECT_NAME";
