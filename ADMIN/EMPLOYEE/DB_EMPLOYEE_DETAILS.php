...
if($_REQUEST['option']=='company_datatable')
{
    ...
    while($row=mysqli_fetch_array($cmp_details)){
        $cmpdetails[]=array($row['EMPLOYEE_NAME'],$row['CP_LAPTOP_NUMBER'],$row['CP_CHARGER_NUMBER'],$row['CP_BATTERY_SERIAL_NUMBER'],$row['CP_LAPTOP_BAG_NUMBER'],$row['CP_MOUSE_NUMBER'],$row['CPD_DOOR_ACCESS'],$row['CPD_ID_CARD'],$row['CPD_HEADSET']);
    }
    $cmp_properties='<table width=1000 border=1 cellpadding=0 cellspacing=0 colspan=3px  style="border-collapse:collapse;">
//        <caption style="caption-side: left;font-weight: bold;">DETAILS</caption><sethtmlpageheader name="header" page="all" value="on" show-this-page="1"/>
    ...
    $finaltable= '<html><body><table><tr><td style="text-align: center;"><div></div></td></tr><tr><td><h2><div style="font-weight: bold;margin-bottom: 5cm;">' .$cmp_properties.'</div></table></body></html>';
